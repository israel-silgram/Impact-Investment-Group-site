...
| `ui/disclosure` | no | nothing. The 21 "disclosure" hits in `src/` are the word in prose and the `disclosure-content` CSS class, which `ui/accordion.tsx` uses and which stays | **deleted** |
...
**The merge was CLEAN. There were no conflicts anywhere**, including in `src/routes/register.$role.tsx`, which wave 414 also edited and where the brief expected one: 411 appended a block at the end of the section and 414's phone work is elsewhere in the file, so `ort` resolved it by itself. **Every part of 414's work on that page is untouched** (the sticky bottom bar, the safe area, the type floors, the target sizes), and the mobile gate re-run confirms it rather than the reading.
...
The re-checker's 3.9x was measured on a **synthetic flat frame**. This site's brand artwork is a soft-edged raster with an anti-aliased alpha ramp, which is the case lossless WebP is worst at. Four further encoder settings were measured on the three brand files and none is usable: `lossless quality=100` and `quality=0` are both larger still, `exact=False` is byte-identical, and `alpha_quality=50` is smaller only by degrading the mask the whole fix exists to preserve.
...
- **`scripts/wave411-screenshots.py`** came in with the merge and was **not run**. It was written against the navy card on the pre-412 dark site and its assertions are about a surface that no longer exists; wave 411's own report is the record of it. The card is covered by all four standing gates instead, which is stricter. **Proposal 5**: retire or rewrite it.
...
4. **The QR image's `alt` says "Impact Investment Group"; `siteName` says "Impact Investment Platform".** Wave 411's string, left verbatim under the no-new-strings rule. **Recommended: change it to "Impact Investment Platform" in the next wave.**
5. **`scripts/wave411-screenshots.py` asserts a surface that no longer exists.** **Recommended: delete it; the four standing gates cover the card better.**
