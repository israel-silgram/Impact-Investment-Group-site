...
export function SuccessState({ role }: { role: RegisterRoleContent }) {
  ...
  return (
    <div
      ref={panel}
      tabIndex={-1}
      className="rounded-[var(--radius-panel)] border border-teal-600 bg-tint-teal p-6 sm:p-8 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-teal-600"
    >
      <div role="status">
        ...
        <h2 className="mt-5 font-heading text-[26px] font-bold text-ink">{role.success.heading}</h2>
        <p className="measure mt-3 text-[15px] leading-relaxed text-ink-muted">
          {role.success.body}
        </p>
      </div>

      <p className="eyebrow mt-8 text-teal-600">What happens next</p>
      <ol className="mt-4 flex flex-col gap-3">
        {role.success.next.map((line, index) => (
          <li key={line} className="flex items-start gap-3">
            ...
            <span className="text-[15px] leading-relaxed text-ink-muted">{line}</span>
          </li>
        ))}
      </ol>
